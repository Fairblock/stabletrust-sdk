...
export function unpaddedBase64(buf) {
    var encodedBuf = Buffer.from(buf).toString("base64");
    var lastIndex = encodedBuf.length - 1;
    while (encodedBuf[lastIndex] === "=") {
        lastIndex--;
    }
    return encodedBuf.slice(0, lastIndex + 1);
}
export function unpaddedBase64Buffer(buf) {
...
/*
    ...
    ["he.", "ll.", "o .", "wo.", "rl.", "d."]
 */
export function chunked(input, chunkSize, suffix) {
    if (suffix === void 0) { suffix = ""; }
    var output = [];
    var currentChunk = "";
    for (var i = 0, chunks = 0; i < input.length; i++) {
        currentChunk += input[i];
        var posInChunk = i - chunks * chunkSize;
...
export function sliceUntil(input, searchTerm) {
